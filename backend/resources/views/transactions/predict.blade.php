...
<head>
    ...
    <title>Predict Fraud - FraudShield</title>
...
<body class="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 min-h-screen">
            ...
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center space-x-8">
                    ...
                    <div class="flex items-center space-x-1">
                        ...
                        <a href="{{ route('transactions.predict') }}" class="flex items-center space-x-2 px-4 py-2 rounded-lg transition-all duration-200 bg-gradient-to-r from-purple-600 to-pink-600 shadow-lg shadow-purple-500/25">
                            <span>🔮</span>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="space-y-6">
            <h1 class="text-3xl font-bold text-white">Fraud Prediction</h1>
            ...
            @if(isset($prediction))
                <div class="bg-slate-800/50 backdrop-blur-lg border border-slate-700 rounded-xl p-6 hover:border-purple-500/50 transition-all duration-300">
                    <h2 class="text-xl font-semibold text-white mb-4">Prediction Result</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-slate-700/50 p-4 rounded-lg border border-slate-600">
                            <h3 class="text-sm font-medium text-slate-300">Prediction</h3>
                            <p class="text-2xl font-bold {{ $prediction['prediction'] == 'fraud' ? 'text-red-400' : 'text-green-400' }}">
                                {{ ucfirst($prediction['prediction']) }}
                            </p>
                        </div>
                        <div class="bg-slate-700/50 p-4 rounded-lg border border-slate-600">
                            <h3 class="text-sm font-medium text-slate-300">Risk Score</h3>
                            <p class="text-2xl font-bold {{ $prediction['risk_score'] > 0.5 ? 'text-red-400' : 'text-green-400' }}">
                                {{ number_format($prediction['risk_score'], 2) }}
                            </p>
                        </div>
                        <div class="bg-slate-700/50 p-4 rounded-lg border border-slate-600">
                            <h3 class="text-sm font-medium text-slate-300">Confidence</h3>
                            <p class="text-2xl font-bold text-purple-400">
                                {{ number_format($prediction['confidence'], 2) }}
                            </p>
                        </div>
                        <div class="bg-slate-700/50 p-4 rounded-lg border border-slate-600">
                            <h3 class="text-sm font-medium text-slate-300">Fraud Probability</h3>
                            <p class="text-2xl font-bold text-orange-400">
                                {{ number_format($prediction['probabilities']['fraud'], 2) }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-slate-800/50 backdrop-blur-lg border border-slate-700 rounded-xl p-6 hover:border-purple-500/50 transition-all duration-300">
                ...
                <form method="POST" action="{{ route('transactions.predict.submit') }}" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                        ...
                        <div>
                            ...
                            <input
                                type="number"
                                name="Time"
                                value="{{ old('Time', $input['Time'] ?? time()) }}"
                                required
                                class="w-full px-4 py-2 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-slate-400"
                        ...
                        <div class="md:col-span-2">
                            ...
                            >
                                <option value="random_forest" {{ old('algorithm', $algorithm ?? 'random_forest') == 'random_forest' ? 'selected' : '' }}>Random Forest (99.93% accuracy)</option>
