...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="space-y-6">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Fraud Prediction</h1>
        ...
        @if(isset($prediction))
            <div class="bg-white dark:bg-slate-800/50 backdrop-blur-lg border border-gray-200 dark:border-slate-700 rounded-xl p-6 hover:border-purple-500/50 transition-all duration-300">
                <h2 class="text-xl font-semibold text-slate-900 dark:text-white mb-4">Prediction Result</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-gray-50 dark:bg-slate-700/50 p-4 rounded-lg border border-gray-200 dark:border-slate-600">
                        <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Prediction</h3>
                        <p class="text-2xl font-bold {{ $prediction['prediction'] == 'fraud' ? 'text-red-400' : 'text-green-400' }}">
                            {{ ucfirst($prediction['prediction']) }}
                        </p>
                    </div>
                    <div class="bg-gray-50 dark:bg-slate-700/50 p-4 rounded-lg border border-gray-200 dark:border-slate-600">
                        <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Risk Score</h3>
                        <p class="text-2xl font-bold {{ $prediction['risk_score'] > 0.5 ? 'text-red-400' : 'text-green-400' }}">
                            {{ number_format($prediction['risk_score'], 2) }}
                        </p>
                    </div>
                    <div class="bg-gray-50 dark:bg-slate-700/50 p-4 rounded-lg border border-gray-200 dark:border-slate-600">
                        <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Confidence</h3>
                        <p class="text-2xl font-bold text-purple-400">
                            {{ number_format($prediction['confidence'], 2) }}
                        </p>
                    </div>
                    <div class="bg-gray-50 dark:bg-slate-700/50 p-4 rounded-lg border border-gray-200 dark:border-slate-600">
                        <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Fraud Probability</h3>
